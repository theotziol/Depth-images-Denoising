...
x3 = Conv2D(n3, (f3, f3),padding = padding,)(x2)



m = Model(input,x3, name = 'SRCNN_{filter0}_{filter1}_{filter2}_in_{a}_{b}_{c}'.format(filter0 = n1,filter1 = n2,filter2 = n3, a=f1,b=f2,c = f3))
m.summary()

m.compile(optimizer=optimizer, loss= 'mse', metrics=[psnr])

os.chdir(save_model)
tf.keras.utils.plot_model(model = m,to_file= 'SRCNN_{}_{}_{}.png'.format(n1,n2,n3), show_shapes = True,show_dtype = True)
early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', mode="min", patience=18,verbose = 2, restore_best_weights= True)

start = timeit.default_timer()
history = m.fit(x,y, batch_size= batch, epochs = 120,validation_split =0.1, verbose = 2, callbacks = early_stop, shuffle = True )
stop = timeit.default_timer()
print('Time: ', stop - start, 'Time minutes:' ,(stop - start)//60)  
m.save('SRCNN_{filter0}_{filter1}_{fi3}_{a}_{b}_{c}'.format(filter0 = n1,filter1 = n2,fi3 = n3,a=f1,b=f2,c = f3))
...
os.chdir(save_model)
f = open('SRCNN_{filter0}_{filter1}_{filter2}_{a}_{b}_{c}.txt'.format(filter0 = n1,filter1 = n2,filter2 = n3 ,a=f1,b=f2,c = f3),'w')
